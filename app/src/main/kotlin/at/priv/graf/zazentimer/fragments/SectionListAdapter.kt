...
class SectionListAdapter(
    ...
    override fun onBindViewHolder(@NonNull holder: ViewHolder, position: Int) {
        ...
        val name: String = if (!section.name.isNullOrEmpty()) {
            section.name!!
        } else {
        ...
        val info: String = if (section.bellcount == 1) {
            durationStr + holder.itemView.context.getString(R.string.section_info_string_1_sg)
        } else {
            val partial = durationStr + String.format(holder.itemView.context.getString(R.string.section_info_string_1_pl), section.bellcount) + " "
    ...
    fun insertItem(position: Int, section: Section) {
