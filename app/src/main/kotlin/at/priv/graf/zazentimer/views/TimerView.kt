    ...
    constructor(
        ...
        override fun onDraw(canvas: Canvas) {
            ...
            canvas.drawArc(
                rectFull,
                f2,
                (angleStart + ((angleEnd - angleStart) * getPrevStartSeconds() / getSessionTotalSeconds()).toInt()) - f2,
                false,
                paintRemaining,
            )
            ...
            canvas.drawArc(
                rectFull,
                prevStartSeconds.toFloat(),
                (angleStart + ((angleEnd - angleStart) * getCurrentStartSeconds() / getSessionTotalSeconds()).toInt()) -
                    prevStartSeconds.toFloat(),
                false,
                paintPrev,
            )
            ...
            canvas.drawArc(
                rectFull,
                currentStartSeconds.toFloat(),
                (angleStart + ((angleEnd - angleStart) * getNextStartSeconds() / getSessionTotalSeconds()).toInt()) -
                    currentStartSeconds.toFloat(),
                false,
                paintCurrent,
            )
            ...
            canvas.drawArc(
                rectFull,
                nextStartSeconds.toFloat(),
                (angleStart + ((angleEnd - angleStart) * getNextEndSeconds() / getSessionTotalSeconds()).toInt()) -
                    nextStartSeconds.toFloat(),
                false,
                paintNext,
            )
            ...
            val sessionElapsedSeconds =
                (
                    (angleStart + ((angleEnd - angleStart) * getSessionElapsedSeconds() / getSessionTotalSeconds())) *
                        3.1415 /
                        180.0
                ).toDouble()
            ...
            if (showTimeMode == 0) {
                str =
                    String.format(
                        Locale.getDefault(),
                        "%1$02d:%2$02d",
                        Integer.valueOf(sectionElapsedSeconds / 60),
                        Integer.valueOf(sectionElapsedSeconds % 60),
                    )
        ...
        fun getColorMarkerLine(): Int = colorMarkerLine

        fun getColorMarkerTriangle(): Int = colorMarkerTriangle

        fun getColorRingPrev(): Int = colorRingPrev

        fun getColorRingCurrent(): Int = colorRingCurrent

        fun getColorRingNext(): Int = colorRingNext

        fun getColorRingRemaining(): Int = colorRingRemaining

        fun getPrevStartSeconds(): Float = floatProperties[1]

        fun getSessionTotalSeconds(): Float = floatProperties[0]

        fun getCurrentStartSeconds(): Float = floatProperties[2]

        fun getNextStartSeconds(): Float = floatProperties[3]

        fun getNextEndSeconds(): Float = floatProperties[4]

        fun getSessionElapsedSeconds(): Float = floatProperties[5]
