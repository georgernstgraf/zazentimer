...
- **Matrix runs with `-no-snapshot-save` to avoid self-poisoning baselines**: If the matrix saves on exit (default), each API's post-test state overwrites the clean baseline — and a post-test snapshot can be non-resumable (API 34 hung on resume: qemu 0 CPU, never booted). Baselines are written ONLY by `create-emulator-snapshots.sh`; `run-instrumentation.sh` fast-boots from them with `-no-snapshot-save` and never overwrites.
- **Snapshot saved after a test run can be non-resumable**: Even a graceful save after a PASSING run can capture guest state that hangs on resume. Never trust a post-test snapshot as a baseline; regenerate via `create-emulator-snapshots.sh` (clean boot → configure → provision → save quiescent state).
