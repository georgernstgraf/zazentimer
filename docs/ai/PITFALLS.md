...
- **`Assume.assumeTrue()` produces silent false-greens in instrumented tests**: When `AndroidJUnitRunner` skips a test via `org.junit.Assume`, it reports `OK (N tests)` with no failure — the dots line may show fewer (or zero) characters but the process exits 0. If fixture provisioning depends on `adb push` (which fails on API 30 scoped storage), an `Assume.assumeTrue(file.exists())` skip yields `OK (4 tests)` with 0 dots — a false green. Never use `Assume` to guard fixture availability; self-provision fixtures from `app/src/androidTest/res/raw/` (see #68). The `FailOnAssumptionSkipListener` RunListener crashes the process on any assumption-skip as a future-regression guard.
- **GPU/SwiftShader log lines interleave into `am instrument` stdout**: On API 34 (and possibly others), `_glBindAttribLocation: bind attrib 0 name position` appears appended to the per-class result dots line (e.g. `ManageBellsTest:.s_glBindAttribLocation: bind attrib 0 name position`), breaking any stdout-based dot/character parser. Do NOT parse `am instrument` stdout dots for assumption-skip or pass/fail detection; use an in-process `RunListener` (`-e listener <FQCN>`) instead. (See #289, api34-2026-06-23 log.)
- **Popup menu animation is not tracked by Espresso idle**: Overflow-menu / popup-menu rendering happens on the UI thread animation loop, not on any tracked idling resource. `onIdle()` does NOT wait for the popup to finish animating. Always use a retry/poll loop (UiAutomator `waitForExists` + Espresso check retries) when matching items inside a popup. `ScreenRobot.clickToolbarOverflowItem` wraps this in a 3-attempt retry loop (see #293).
- **Changing the fdroiddata commit requires rebuilding the reference APK**: the APK embeds the git commit hash (`META-INF/version-control-info.textproto`) and commit count (`BuildConfig.VERSION_DISPLAY = "$versionName+$commitCount"`). So even a metadata-only commit (e.g. fastlane summary fix) produces a different `classes.dex` than the reference APK built at the old commit. When updating the `commit:` hash in the fdroiddata MR, you MUST also move the version tag to the new commit and rebuild the reference APK via `fdroid-apk.yml` (`workflow_dispatch` with `tag=vX.Y.Z`), then retry the F-Droid pipeline. (See MR !39945.)
