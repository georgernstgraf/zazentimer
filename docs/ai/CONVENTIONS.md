...
- `sanitizeBellUris()` runs at every startup in `ZazenTimerActivity.onCreate()` (and after manual restores). It is the sole bell-sync function. Syncs builtin bells with `BuiltinBells.definitions()` (pure config), removes orphaned bells (reassigning sections to demo), and enforces strict 1:1 mapping for custom bells with disk.
...
- **Session rank persistence**: Session ranks are saved in TWO places: (1) `MainFragment.onPause()` via an application-scoped `CoroutineScope` — the lifecycle safety net that fires on navigation/pause; (2) `MainFragment.suspendUpdateSessionList()` at the TOP (before clearing and reloading from DB) — prevents the in-memory drag order from being overwritten by stale DB data when actions like Add/Delete/Duplicate call `updateSessionList()`. Both assign `sessions[i].rank = i` and call `sessionRepo.updateSession()`.
...
- **After backup restore, call `sanitizeBellUris()`**: `SettingsFragment.doRestore()` calls `dbOperations.sanitizeBellUris()` after a successful backup restore. This ensures bell URIs are normalized for the current app's package name, builtin bells match `BuiltinBells`, and custom bells have a strict 1:1 mapping with files on disk. Never skip this step — the database may have stale URIs from a different build type or app version.

## Bell Data Architecture
- **DB is the single source of truth for bells.** No in-memory cache (the old `BellCollection` singleton was removed in #270 — it raced with the DB and caused FK crashes). All bell reads go through `BellDao`/`BellRepository`/`DbOperations` (all `suspend`).
- **`BuiltinBells` is pure config, not state.** `audio/BuiltinBells.kt` is an `object` with `definitions(): List<BellSeed>` (8 built-in bells mapping raw resource IDs to name string resources), `resourceUri(context, rawResId)`, and demo-bell constants. No `initialize()`, no mutable fields, no `Context` state.
- **`BellEntity` is the only bell representation.** There is no separate `Bell` business object. `GongListAdapter` is `ArrayAdapter<BellEntity>`, `Audio.playAbsVolume(uri: Uri?, volume: Int)` takes the URI directly, and `BellPlayer` uses `BellEntity.uri` from `getBellById`.
- **Demo bell fallback throws, never returns 0.** `SessionRepository.fallbackBellId()` and `SectionRepository.fallbackBellId()` throw `error("No builtin bells in database")` if no demo bell exists. The old `?: 0` silently corrupted FK references.
...
- **Overflow-menu interactions use retry-based opening**: `ScreenRobot.clickToolbarOverflowItem` calls `onIdle()` first, then retries the open-and-find sequence 3 times (re-opening the overflow if the target item isn't found). After screen-transition navigations that precede overflow interactions (e.g. `SettingsPage.clickManageBells`, `SessionEditPage.clickAddSection`), call `onIdle()` so fragment transactions settle before the next overflow open. (See #293.)
...
- **Use `inRoot(isDialog())` for AlertDialog interactions**: On API 36+, the system enforces edge-to-edge (`EDGE_TO_EDGE_ENFORCED`), which can cause activity windows to lose focus when `AlertDialog` appears. Espresso's default root matcher requires window focus, causing `RootViewWithoutFocusException`. Use `.inRoot(isDialog())` to target the dialog root directly. Import: `import androidx.test.espresso.matcher.RootMatchers.isDialog`. **Apply to EVERY dialog-button click**, not just the obvious ones — `android.R.id.button1`/`button2` (AlertDialog positive/negative), `R.string.action_delete`/`action_ok`/`action_cancel` (text-matched dialog buttons), and any `TimePickerDialog`/`DatePickerDialog` confirmation. Existing violations: `SectionEditTest.kt:70` and `MeditationServiceTest.kt:100-108` were fixed in #292. Audit any new dialog click via `rg "withId\(android\.R\.id\.button[12]\)|withText\(R\.string\.action_(delete|ok|cancel)\)" app/src/androidTest/` — every match must chain `.inRoot(isDialog())`.
- **Negative assertions need positive corroboration**: a `verifyBellNotListed(name)` alone does NOT prove "import failed correctly" — it could pass if the import never started (button click missed, SAF intent never delivered, etc.). Pair every negative assertion with positive evidence: (1) production catch block must `Log.w(TAG, "...", e)`; (2) test asserts a side effect that ONLY the failure path produces (e.g., partial file deleted by importer's `finally`, logcat contains the `Log.w` line, no process crash). Reaching assertions without process crash IS proof the catch ran (uncaught exceptions crash the test process). (See #291.)
- **Indirect Toast verification pattern (API 31+)**: Android Toast windows are invisible to UiAutomator (PITFALLS). To verify a Toast-based failure path: (1) production catch block writes `Log.w(TAG, "Bell import failed", e)`; (2) test asserts side effects (bell NOT in list + partial file absent); (3) `rg "ZMT_<TAG>" logs/api<level>-<date>-logcat.txt` shows the catch-block log fired. No need for Toast-text assertion; the log + side effects are sufficient and far more reliable.
- **Catch `BellImportException` around every `BellValidator.validate()` in background paths**: `BellValidator.validate` throws on invalid audio. In UI-initiated flows (e.g., `ManageBellsFragment` import handler) the catch shows a toast — fine. In background startup flows (`BellSanitizer.importOrphanedBellFiles`, future sanitize paths) the catch must `Log.w`, delete the corrupt file, and continue — an uncaught throw here bricks the app on every launch. (See #291.)
- **Bell DB tests**: Always seed at least one builtin bell via direct `bellDao.insert(BellEntity(...))` or `dbOperations.insertBell(...)`. For `fallbackBellId`/demo-bell tests, seed the demo bell by name: `context.getString(BuiltinBells.DEMO_BELL_NAME_RES)` with `isBuiltin=true`. Insert custom bells with `isBuiltin=false` and `file://` URIs.
