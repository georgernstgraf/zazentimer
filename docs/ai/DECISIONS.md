...
## 2026-06-17: Keep `runBlocking`-in-`onPause` as-is (#270 follow-up deferred)
- **Choice**: Leave the `runBlocking { … }` DB writes in the editing fragments' `onPause` unchanged. Do NOT async-ize them now.
- **Reason**: The only place this pattern actually broke something was `MainFragment.onPause` rank persistence, already fixed in #273 via drop-time `onDragEnd` (so that `onPause` save is now just a redundant safety net). The remaining `runBlocking`s work — edits persist; the one real deadlock (`withTransaction`-in-`runBlocking`) was already removed in 768ed2b. Converting is marginal: the main win is less main-thread jank on navigation (~ms), weighed against new app-scope infra + a subtle async write/read-ordering risk concentrated in the two **load-bearing** saves (`SessionEditFragment` name/desc, `SectionEditFragment`), where the jank benefit is smallest and the risk highest. Poor risk/reward → defer.
- **Revisit trigger**: re-open this if (a) navigation jank becomes noticeable, (b) a `runBlocking`-on-main deadlock reappears, or (c) someone re-adds `withTransaction` inside one of these. Then convert via an **app-scoped `CoroutineScope(SupervisorJob()+Dispatchers.IO)`**, capturing UI text synchronously in `onPause` (while `binding` is valid) then `appScope.launch { db.write() }` — the scope survives fragment destruction (back-pop), which a naive `lifecycleScope.launch` does NOT. The repo `withIdling` wrappers keep Espresso synced; add a synchronous-increment `launchTracked()` helper if tests flake.
- **`runBlocking` inventory (9 sites) for whoever revisits**: 4 in `onPause` — `MainFragment:169` (assignRanks, now redundant behind #273), `SessionEditFragment:204` (updateSection×N + updateSession — load-bearing), `SectionEditFragment:131` (audio.release) + `:138` (updateSection — load-bearing). 3 in `SectionEditFragment` UI listeners — `:186`/`:273` (synchronous bell **reads** for spinner/play; need pre-loading, not a trivial swap) + `:299` (bell-change **write**; trivially `appScope.launch`-able). 2 test-only in `ZazenTimerActivity.resetDatabaseForTest` (`:456`,`:469`) — leave as-is.
- **Cheap optional slice** (not done): `MainFragment.onPause`'s `assignRanks` is now redundant behind `onDragEnd` and could be deleted for a real complexity cut at near-zero risk — but left in as a belt-and-braces safety net.
