...
## 2026-06-11: Long-press to edit session + fix dual-selection (#246)
- **Choice**: (a) Disable `isLongPressDragEnabled()` in `SessionTouchHelperCallback` and use drag-handle-initiated reorder to free long-press for navigation to session edit. (b) Replace targeted `notifyItemChanged(previous/selectedPosition)` with full-list refresh in `setSelectedPosition()` and the click listener to fix dual-selection.
- **Reason**: Long-press was monopolized by `ItemTouchHelper` for drag. Freeing it enables direct navigation. The full-refresh selection pattern eliminates the stale-`selectedPosition` bug after `removeItem`/`insertItem`/`moveItem` shift the backing list without updating selection state — `notifyItemChanged(previous)` could target an OOB or wrong index.
...
- **Choice**: `SessionTouchHelperCallback` overrides `clearView(...)` → `MainFragment.onDragEnd()` → `viewLifecycleOwner.lifecycleScope.launch { sessionRepo.assignRanks(sessions) }` (async) writes ranks the instant the user releases the grip handle. `onPause()`'s `assignRanks` is kept as an idempotent safety net, launched on the application-scoped `CoroutineScope` so the write survives fragment destruction.
...
## 2026-06-20: Migrate remaining production `runBlocking` callsites to an app-scoped CoroutineScope (#270 follow-up)
- **Choice**: Replaced the deferred `runBlocking` DB/audio writes in `MainFragment.onPause()`, `SessionEditFragment.onPause()`, and `SectionEditFragment` with async launches on a Hilt-provided application-scoped `CoroutineScope(SupervisorJob() + Dispatchers.IO)`. UI values are captured synchronously while `binding` is still valid; the actual repo calls run in the application scope so they survive fragment destruction (back-pop), which `lifecycleScope.launch` would not. `SectionEditFragment` now preloads the current bell in `fillViewFromData()` and caches both the `BellEntity` and its `Uri`, eliminating the synchronous bell reads that previously blocked the spinner and play-button listeners.
- **Reason**: The revisit trigger from the 2026-06-17 decision was met — the codebase had stabilized after #268 (DbOperations dissolution), and the remaining `runBlocking` calls were the last synchronous DB/audio operations on the main thread in production code. Converting them removes main-thread jank on navigation and avoids the lifecycle risk of `lifecycleScope` jobs being cancelled when the fragment is popped.
- **Converted sites (7 production)**: `MainFragment.onPause()` `assignRanks`; `SessionEditFragment.onPause()` section-rank + session name/description save; `SectionEditFragment.onPause()` `audio.release()` + `sectionRepo.updateSection()`; `SectionEditFragment.selectBellForSection()` + `installPlayGongListener()` (via preloaded/cached bell); `SectionEditFragment.installBellSelectionListener()` bell-change write. The 2 test-only `runBlocking` calls in `ZazenTimerActivity.resetDatabaseForTest` remain unchanged.
- **Infrastructure**: New `di/AppCoroutineScopeModule.kt` provides `@Singleton CoroutineScope = SupervisorJob() + Dispatchers.IO`. The scope is field-injected into `MainFragment`, `SessionEditFragment`, and `SectionEditFragment`.
- **Idling resources**: Existing repo `withIdling { ... }` wrappers already increment/decrement the `CountingIdlingResource` inside each suspend DB call, so Espresso stays synchronized without an additional `launchTracked()` helper.
- **Tradeoff**: Slightly more complex lifecycle reasoning (must capture UI values locally before launching), but the Hilt singleton scope is simple and the fragment code changes are localized. The redundant `assignRanks` safety net in `MainFragment.onPause()` is preserved.

## 2026-06-19: Dissolve DbOperations façade — DatabaseOwner + 4 @Inject repositories (#268)
- **Choice**: Deleted the `DbOperations` god-class façade. All consumers inject the specific repository (`SessionRepository`/`SectionRepository`/`BellRepository`/`BellSanitizer`) or `DatabaseOwner` directly via Hilt. `DatabaseOwner` (`@Singleton @Inject constructor(@ApplicationContext)`) owns the Room `AppDatabase` lifecycle (build with migrations/callback, WAL-checkpoint-on-close, reopen, `getActualDatabaseVersion`). The 4 repos are `@Singleton @Inject constructor(databaseOwner, @ApplicationContext)` and fetch DAOs **dynamically** from `DatabaseOwner` on each call (never cache).
- **Reason**: `DbOperations` was a 177-line god-class that 13 production + 7 test consumers injected — every screen depended on the whole DB surface. Worse, it wasn't a pure façade: it **owned the Room lifecycle** (`close`/`reopen`/`getActualDatabaseVersion`), which the issue body (#268) didn't account for. Dissolving it gives each consumer exactly the DB surface it needs (1–3 repos) and a clean `DatabaseOwner` for lifecycle.
- **Reopen-safe design (critical)**: Hilt singletons can't be rebuilt, but `close()`/`reopen()` (backup-restore flow) recycle the `AppDatabase` connection. Repos must NOT cache DAO references — they fetch via `databaseOwner.sessionDao()` etc. on each use, so after `reopen()` they always hit the current `AppDatabase`. This is why repos take `DatabaseOwner` rather than injected DAOs.
- **Phased approach** (6 commits): P0 Hilt foundation (no consumer changes — de-risked the Hilt graph in isolation) → P1 single-repo consumers → P2A multi-repo fragments → P2B last prod consumers (incl. `DemoSessionCreator` made `@Inject`) → P3 test consumers → P4 delete `DbOperations` + update `ARCHITECTURE.md`. `DbOperations` stayed alive through P3 as a fallback.
...
- **Revisit trigger**: a quieter moment, OR adopting a detekt `FunctionNaming`/ktlint rule to enforce snake_case for test sources + layout IDs (the issue's stated trigger). Then do layout IDs and test methods as **separate commits** (never both at once — diff becomes unreviewable) and use IDE refactoring for `R.id` references.

## 2026-06-24: Retry-based overflow-menu opening in ScreenRobot (#293)
- **Choice**: Rewrote `ScreenRobot.clickToolbarOverflowItem` to (1) call `onIdle()` before opening, (2) wrap open-and-find in a 3-attempt retry loop that re-opens the overflow if the target menu item isn't found, and (3) prefer Espresso's `openActionBarOverflowOrOptionsMenu` with UiAutomator fallback. Added `onIdle()` after screen-transition navigations in `SettingsPage.clickManageBells` and `SessionEditPage.clickAddSection`.
- **Reason**: All 16 overflow-menu interactions in the test suite routed through this single method. Under sustained host load, the initial overflow click could be lost (slow rendering, click on not-yet-ready view) with no retry of the open step — the target item never appeared and the test threw `NoMatchingViewException`. Intermittent flakes observed on API 33 (SectionEditTest/SettingsTest) and API 29 (ManageBellsTest import_button chain).
- **Considered**: Only adding `onIdle()` calls (helps DB sync but doesn't fix lost-click race), increasing Espresso idle timeout (masks the race rather than fixing it), adding a custom IdlingResource for popup animation (popup menu rendering isn't on a tracked thread — can't be reliably tracked).
- **Tradeoff**: On total failure (3 retries exhausted), `device.pressBack()` between attempts could theoretically pop a fragment if the menu never opened. In practice the menu IS open but slow to render, so pressBack correctly dismisses it before retry. The final unconditional `perform(click())` preserves clear `NoMatchingViewException` diagnostics if all retries fail.
