...
2. [ ] **#270 follow-up** (deferred) — migrate remaining `runBlocking` callsites in `SectionEditFragment` + `MainFragment.onPause()` to async. See DECISIONS.md "Keep runBlocking-in-onPause as-is" for the full 9-site inventory + app-scope conversion recipe + revisit trigger.

3. [ ] **Env followup (claw)** — API 34 freezer skip-check fooled by `setting=disabled` vs flag-not-applied on fast-boot resume (remedy: `--cold-boot` or re-baseline to `setting=null`); API 36 `system_server` crash on claw. Both documented in PITFALLS. Not code regressions.
